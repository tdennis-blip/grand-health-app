...
- **Migrations without the tunnel (preferred, added 2026-07-29):** run `scripts/migrate-remote.sh <file>.sql` — fires a one-off ECS Fargate task (`grand-health-staging-migrate`) that runs `scripts/run-migrations.mjs` inside the VPC against RDS using the owner role (`SERVICE_ROLE_DATABASE_URL` → `MIGRATE_DATABASE_URL`). No bastion, no port-forward. Needs a one-time `cdk deploy` to create the task def; see the 2026-07-29 session log. The runner takes `MIGRATE_FILE=<one file>` (applies just that file, idempotent-safe) or applies all pending tracked in `public.schema_migrations`.
- **Bastion tunnel (legacy fallback):** current bastion **i-0c2809c11877c15f9** (re-discover if `TargetNotConnected`). `aws ssm start-session --target i-0c2809c11877c15f9 --document-name AWS-StartPortForwardingSessionToRemoteHost --parameters '{"host":["<RDS endpoint above>"],"portNumber":["5432"],"localPortNumber":["5432"]}'` then `export DIRECT_DATABASE_URL="postgresql://grandhealth:<pw>@localhost:5432/grandhealth?sslmode=require"`. **It keeps dropping** because the repo lives in an **iCloud** folder — see "move to ~/dev" below.
...
- **Non-admin clinician visibility gates — VERIFIED PASS (2026-08-06):** created/used a non-admin clinician and confirmed the admin-only surfaces are correctly hidden — no Audit-log nav (and `/clinician/audit` redirects away), no danger zone on patient pages, no staff toggle in Add patient, no admin/deactivate controls on Team. All gated correctly.
- **⚠️ Still NOT verified:** the **TOTP-less clinician → `/mfa-setup` redirect** — every current clinician already has TOTP enrolled; needs a fresh un-enrolled clinician to test.
...
- **🐛 BUG FOUND + FIXED (2026-08-06, commit `3048478`):** the LOGGED SETS card header showed **"Invalid Date"**. Root cause: `esl.log_date` came back from Postgres as a JS `Date` object, and `page.tsx` did `String(log_date).slice(0,10)` → `"Wed Aug 06"` → `new Date("Wed Aug 06T00:00:00")` = Invalid Date. Fix: cast `log_date::text` in both the sets and feedback queries (always `YYYY-MM-DD`) + defensive render guard (shows "Undated" if unparseable). Typecheck clean; pushed to main → CI deploy.
...
**⭐ Training (migration `0038_rep_range_and_workout_feedback.sql`, ✅ APPLIED to staging 2026-07-29):**
- **Rep ranges:** `session_sets.reps_min`/`reps_max` (backfilled from `reps`; legacy `reps` kept synced to the top of range for 1RM analytics). Session builder shows a min–max pair; patient logs actual reps; target shows "8–12".
- **Per-session-exercise coaching note:** `session_exercises.coach_note`, editable inline per exercise in the session builder, shown to the patient (separate from the exercise-library default note).
- **End-of-workout feedback:** new `session_feedback_logs` (RPE 1–10 + comment, per patient/session/date, RLS + ct_restrict). Patient "How did that feel?" section; comment is forwarded to the patient's trainer (primary clinician → fallback first active care-team clinician) as an in-app message via `saveWorkoutFeedback`. RPE+comment also surface on the clinician "Logged workouts" page. Commit `976ea7c`.

**⭐ Diet — intent-based calorie planning (migration `0039_daily_activity_intents.sql`, ✅ APPLIED 2026-07-29):**
- New activity mode **`intent`**: `goal = RMR × base_multiplier(~1.2) + stated-intent active kcal + deficit`. Avoids the double-count you'd get from a full activity multiplier + added activity. Implemented as a new active-kcal *source* in the existing dynamic pipeline (`lib/diet.ts` deriveTargets + `getIntentKcalForDate`).
- **`daily_activity_intents`** table (prescribed yes/no/done + custom activity + expected_kcal, RLS + ct_restrict) and **`patient_profiles.diet_activity_planning`** patient opt-in flag (overrides the clinician's mode with intent when on).
- Patient **daily activity check-in** on the diet screen (`activity-checkin.tsx`): each prescribed session Will do / Already did / Skipping (with MET estimate), plus add a custom activity (minutes→MET or manual kcal). `activity-planning-toggle.tsx` for opt-in. **Note:** with planning on and nothing planned, a rest day drops to base+deficit (intended: rest days eat less).
- **Provider-only "Expected vs. measured burn" panel** (`lib/diet-analytics.ts` + `diet/burn-comparison-panel.tsx`, on the clinician patient page): per-day expected (base + planned) vs. tracker-measured total burn over 21 days, hit-rate ("burned ≥ expected on N of M days"), tracker-days only. No migration. (Diet phase-2 commit pending as of this writing.)

**⭐ Tunnel-free migrations (infra — NEEDS `cdk deploy`):** added a one-off ECS Fargate **migrate task** (`grand-health-staging-migrate`) in `infra/lib/app-runtime.ts` + a self-contained runner `scripts/run-migrations.mjs` (uses the app image; postgres-js is zero-dep so only that module + the migrations dir are copied into the runner image — see Dockerfile). Run with `scripts/migrate-remote.sh <file>.sql`, which `aws ecs run-task`s it in the web service's subnets + security group (so RDS already allows it) and tails the logs. Replaces the drop-prone SSM port-forward. **To activate:** `cd infra && npx cdk deploy -c stage=staging -c withService=true -c domain=staging.mygrandhealth.com -c hostedZone=staging.mygrandhealth.com` (the ONLY new resources are the migrate task def + 3 CfnOutputs — verify the change set adds nothing to the service/RDS). New stack outputs: `MigrateTaskDefArn`, `MigrateSecurityGroupId`, `MigrateSubnets`. Runner modes: `scripts/migrate-remote.sh <file>.sql` (one file), `--baseline` (record all present files as applied, run nothing — do this ONCE to adopt the hand-migrated DB), `--all` (apply all pending tracked in `public.schema_migrations`). Baseline caveat: it marks every `.sql` in the folder applied, incl. the duplicate-numbered pairs (0019/0022/0024 ×2) — safe only because all of them were actually run.
