...
import { getCareTeam, careTeamRole } from "@/lib/care-team";
...
export default async function PatientProfile() {
  ...
  const [[profile], [patient], team] = await Promise.all([
    ...
    // Care team = clinicians assigned to this patient.
    getCareTeam(user.id),
  ]);

  const teamInitials = (m: { firstName: string | null; lastName: string | null }) =>
      ...
      {team.length > 0 && (
        <div className="bg-white rounded-2xl border border-slate-200 p-5">
          ...
          <div className="space-y-3">
            {team.map((m) => {
              const role = careTeamRole(m);
              const isPrimary = m.clinicianId === patient?.primary_clinician_id;
              return (
