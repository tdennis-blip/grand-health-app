  ...
  const [row] = await withAuth(user, (sql) =>
    sql`
      INSERT INTO diet_plans (patient_id, clinic_id, rmr_value, rmr_method, rmr_measured_on, rmr_measured_by,
        activity_multiplier, activity_mode, base_multiplier, activity_credit_pct,
        deficit_kcal, protein_per_kg, carbs_pct, fat_pct, fiber_g, meals_per_day, water_l, notes, updated_at)
      VALUES (${parsed.patientId}, ${patient.clinic_id}, ${parsed.rmrValue ?? null}, ${parsed.rmrMethod ?? null},
        ...
        ${parsed.activityMultiplier.toFixed(2)}, ${parsed.activityMode}, ${parsed.baseMultiplier.toFixed(2)}, ${parsed.activityCreditPct},
        ${parsed.deficitKcal}, ${parsed.proteinPerKg.toFixed(1)},
      ...
      ON CONFLICT (patient_id) DO UPDATE SET
        ...
        activity_multiplier = EXCLUDED.activity_multiplier, activity_mode = EXCLUDED.activity_mode,
        base_multiplier = EXCLUDED.base_multiplier, activity_credit_pct = EXCLUDED.activity_credit_pct,
