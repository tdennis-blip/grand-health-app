...
export default async function StackHistoryPage({
  ...
  const [[patientRow], history] = await Promise.all([
    withAuth(user, (sql) =>
    ...
    getMedicationHistory(id, 200, user),
  ]);
    ...
    <main className="max-w-3xl mx-auto px-6 py-6 space-y-5">
      ...
      <header>
        ...
        <div className="text-xl font-semibold text-slate-900 flex items-center gap-1.5">
          <HistoryIcon size={18} className="text-slate-600" /> Medication history
        </div>
        <div className="text-[12px] text-slate-500 mt-1">
          Every add / change / remove on this patient&apos;s stack.
      ...
      <div className="flex items-baseline justify-between gap-2">
        <div className="min-w-0">
          <div className="text-sm font-semibold text-slate-900 truncate">
            {change.medicationName ?? "(deleted)"}
          </div>
          <div className="text-[11px] text-slate-500">
            {when}
            {change.actorName && ` · ${change.actorName}`}
            {change.actorRole && (
              <span className="text-slate-400"> ({change.actorRole})</span>
            )}
          </div>
        </div>
